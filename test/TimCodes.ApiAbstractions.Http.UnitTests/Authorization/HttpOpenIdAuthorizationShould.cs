...
using TimCodes.ApiAbstractions.Http.Authorization.OpenId;
...
public class HttpOpenIdAuthorizationShould
{
    ...
    public async Task AddBearerAuth()
    {
        ...
        TestFixture.MockHttp.When(HttpMethod.Post, "http://timcodes.net/token")
    ...
    public async Task RenewCachedToken()
    {
        ...
        TestFixture.MockHttp.When(HttpMethod.Get, "http://timcodes.net/RenewCachedToken")
            .WithHeaders("Authorization", "Bearer test")
            .Respond(MediaTypes.Json, "{}");

        var resolver = _client.GetResolver<TestResponse>();

        var request = new HttpApiGetRequest(new Uri("http://timcodes.net/RenewCachedToken"), resolver);
    ...
    public async Task RenewTokenOn401()
    {
        ...
        TestFixture.MockHttp.When(HttpMethod.Get, "http://timcodes.net/RenewTokenOn401")
            .WithHeaders("Authorization", "Bearer test")
            .Respond(HttpStatusCode.Unauthorized, MediaTypes.Json, "{}");
        ...
        var request = new HttpApiGetRequest(new Uri("http://timcodes.net/RenewTokenOn401"), resolver);
